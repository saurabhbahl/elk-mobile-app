...
function extractPreCacheUrls(
  ...
  const addImage = (obj: Record<string, unknown> | unknown, path: string[]) => {
    ...
    if (typeof obj === "string" && obj.startsWith("http")) {
      ...
      typeof obj === "object" &&
      (obj as Record<string, string>).url &&
      typeof (obj as Record<string, string>).url === "string" &&
      (obj as Record<string, string>).url.startsWith("http")
    ) {
      list.push({
        path: [...path, "url"],
        url: (obj as Record<string, string>).url,
      });
    }
  };
  ...
  if (d.app_branding) {
    ...
    addImage(branding.splash_loading_screen_background, [
      "app_branding",
  ...
  if (d.plan_your_trip) {
    ...
    if (trip.hero_image)
      addImage(trip.hero_image, ["plan_your_trip", "hero_image"]);
  ...
  if (d.tips_screen_settings) {
    ...
    if (tipsS.header_icon)
      addImage(tipsS.header_icon, ["tips_screen_settings", "header_icon"]);
  }
  // 3. Home Screen
  ...
  if (hs?.map_preview_image) {
    addImage(hs.map_preview_image, ["home_screen", "map_preview_image"]);
  }

  // General Arrays
  const checkArray = (key: string) => {
    if (d[key] && Array.isArray(d[key])) {
      (d[key] as Record<string, unknown>[]).forEach(
        (item: Record<string, unknown>, idx: number) => {
          addImage(
            item.thumbnail_image || item.featured_image || item.nav_image,
            [
              key,
              String(idx),
          ...
          if (
            ...
            addImage(item.additional_images[0], [
              key,
              String(idx),
              "additional_images",
              "0",
          ...
          if (key === "pois" && item.pin_icon_override) {
            addImage(item.pin_icon_override, [
              key,
              String(idx),
              "pin_icon_override",
            ]);
...
export class SyncManager {
  ...
  private static progressListeners = new Set<
  ...
  static async triggerDeltaSync(): Promise<boolean> {
    ...
    try {
      ...
      if (
        ...
        (Array.isArray(settingsMap.app_branding) &&
          settingsMap.app_branding.length === 0)
      ) {
        console.log(
          "[SyncManager] Missing or corrupt local data. Forcing FULL sync.",
        );
        const res = await this.fetchAndStoreAll();
        console.log(
          "[SyncManager] Background full sync completed. Result:",
          res,
        );
        return res;
      }
      ...
      console.log(
        "[SyncManager] Background delta sync completed. Result:",
        res,
      );
      return res;
    } catch (e) {
  ...
  static async fetchAndStoreAll(
    ...
    if (SyncManager.activeSyncPromise) {
      console.log(
        "[SyncManager] Sync already in progress, awaiting active sync...",
      );
    ...
    const task = (async () => {
      ...
      try {
        ...
        console.log(
          "[SyncManager] Fetching split sync data from endpoints parallelly...",
        );
        const endpoints = [
          "pois",
          "programs",
        ...
        const fetchPromises = endpoints.map(async (endpoint) => {
          try {
            const res = await ApiService.fetchEndpointData<any>(
              endpoint,
              isDelta,
              lastSyncTime,
            );
            ...
            console.error(
              `[SyncManager] Error fetching split endpoint ${endpoint}:`,
              err,
            );
            console.warn(
              `[SyncManager] Fetch failed for ${endpoint}, continuing sync with cached/empty data...`,
            );
        ...
        try {
          if (
            mergedJson.app_branding &&
            (!Array.isArray(mergedJson.app_branding) ||
              mergedJson.app_branding.length > 0)
          ) {
            console.log("[SyncManager] Saving app branding settings early...");
            appRepository.upsertSetting(
              "app_branding",
              JSON.stringify(mergedJson.app_branding),
            );
          }
        } catch (err) {
        ...
        SyncManager.notifyProgress(0.5, "Preparing app photos...");
        ...
        if (totalImages > 0) {
          ...
          const downloadWorker = async () => {
            while (true) {
              ...
              downloadedCount++;
              const pct = 0.5 + (downloadedCount / totalImages) * 0.45;
        ...
        for (const item of recordTypes) {
          if (
            ...
            db.withTransactionSync(() => {
              (item.array as Record<string, unknown>[]).forEach(
                (rec: Record<string, unknown>) => {
                  const id = rec.id ? String(rec.id) : String(Math.random());
                  const lastModified = rec.updated_at
                    ? String(rec.updated_at)
                    : hashString(JSON.stringify(rec));
                  appRepository.upsertRecord(
                    id,
                    item.type,
                    JSON.stringify(rec),
                    lastModified,
        ...
        db.withTransactionSync(() => {
          if (
            isDelta &&
            mergedJson.deleted &&
            typeof mergedJson.deleted === "object"
          ) {
  ...
  static isEventExpired(
    event: any,
    pastEventsVisibility: string | undefined | null,
  ): boolean {
