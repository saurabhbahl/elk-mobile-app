...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    if (
      isFocused &&
      popupData &&
      popupData.popup_enabled &&
      !hasDismissedPopupSession
    ) {
      ...
      >
              ...
              {isValidData(homeData?.map_preview_image?.url) ? (
                ...
                >
                  {isValidData(homeData?.map_view_button_label) ? (
                    ...
                    >
                      <AppText style={styles.viewMapButtonText}>
        ...
        {isValidData(homeData?.programs) ? (
          <Animated.View entering={FadeInUp.duration(300).delay(90)}>
            ...
            >
              {(homeData?.programs || []).map(
                (program: ProgramsData, index: number) => {
        ...
        {isValidData(homeData?.featured_event) ? (
          <Animated.View entering={FadeInUp.duration(300).delay(120)}>
            ...
            {(() => {
              ...
              const eventObj = isEventArray
                ? (homeData?.featured_event as any)[0]
                : homeData?.featured_event;
              if (!eventObj) return null;

              const eventId = eventObj.id;
              const eventIndex = eventsData?.findIndex(
                (e: EventsData) => String(e.id) === String(eventId),
              );
              const targetId =
                eventId ||
                (eventIndex !== undefined && eventIndex !== -1
                  ? eventIndex
                  : 0);
                  ...
                  (trail: TrailsData, index: number) => (
                    ...
                    >
                      {isValidData(trail.trail_name) ? (
                        <AppText style={styles.trailName}>
                          {trail.trail_name}
                        </AppText>
                      ) : null}
                      {isValidData(trail.distance) ? (
                        ...
                        >{`    ${formatTrailDistance(trail.distance)}mi`}</AppText>
                      ) : null}
                    </TouchableOpacity>
                  ),
                )}
        ...
        {(() => {
          const grantLogoUrl =
            typeof homeData?.sponsorship_information?.grant_logo === "string"
              ? homeData?.sponsorship_information?.grant_logo
              : homeData?.sponsorship_information?.grant_logo?.url;
          const grantDetailsHtml =
            homeData?.sponsorship_information?.grant_details;
          ...
          <View style={styles.modalContentWrapper}>
            {popupData ? (
              <TouchableOpacity
                ...
                onPress={() => {
                  const ctaLink =
                    (brandData as any)?.cta_button_link?.link ||
                    popupData.cta_button_link?.link ||
                    (brandData as any)?.cta_button_link?.url ||
                    popupData.cta_button_link?.url;
                  if (ctaLink) {
                  ...
                  >
                    <View style={styles.modalImageOverlay}>
                      ...
                      {isValidData(popupData.popup_title) ? (
                        <AppText style={styles.modalTitleDynamic}>
