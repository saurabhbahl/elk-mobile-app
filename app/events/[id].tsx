...
export default function EventDetailScreen() {
  ...
  const event = React.useMemo(() => {
    return eventsData?.find((e: EventsData) => String(e.id) === String(id));
  }, [eventsData, id]);

  const poiId = React.useMemo(
    () => extractPoiId(event?.location_poi_link),
    [event?.location_poi_link],
  );
  ...
  const poiName =
    relatedPoi?.poi_name || relatedPoi?.title || event?.location_name || null;
  ...
  const eventLinkObj = React.useMemo(() => {
    const raw =
      (event as any)?.registration_ticket_link ||
        ...
        <View style={styles.detailsContent}>
          ...
          {isValidData(event["start_date_&_time"]) ? (
            <View style={[styles.infoRow, { alignItems: "flex-start" }]}>
              ...
              >
                {formatEventDateTime(
                  event["start_date_&_time"] as string,
                  isValidData(event["end_date_&_time"])
                    ? (event["end_date_&_time"] as string)
                    : undefined,
                )}
              </AppText>
            </View>
