  ...
  if (!isValid) {
    return (
      ...
      >
        ...
        <AppText style={{ color: "#CCCCCC", marginTop: 8 }}>
...
export default function LiveCameraScreen() {
  ...
  const { brandData, camerasData, liveCamSettingsData, apiStatus } =
    useAppContentData();
  ...
  const styles = React.useMemo(
    ...
    [colors, fonts, isDark, bgColor, secColor, screenWidth],
  );
  ...
  const cameras = (camerasData || []).filter(
    (cam: CamerasData) => cam.active !== false,
  );
  ...
  const getCameraThumbnail = () => {
    ...
    if (
      ...
      isValidStreamUrl(activeCamera.stream_url, activeCamera.stream_type)
    ) {
      const match = activeCamera.stream_url.match(
        /(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=|live\/))([^&?/"']{11})/,
      );
      if (match && match[1]) {
  ...
  const renderPlayer = () => {
    ...
    if (
      !streamUrl ||
      !isValidStreamUrl(streamUrl, streamType) ||
      hasStreamError
    ) {
      return (
        <View
    ...
    if (streamType === STREAM_TYPES.EMBED) {
      ...
      if (processedStreamUrl.includes("<iframe")) {
        processedStreamUrl = processedStreamUrl.replace(
          /src=["']([^"']+)["']/,
      ...
      {apiStatus === "fetching" ? (
        ...
        >
          ...
          <View style={styles.playerContainer}>
            {activeCamera ? (
              isConnected === false &&
              isValidData(liveCamSettingsData?.offline_message) ? (
                <View
                  style={[
                    styles.playerImage,
                    {
              ...
              >
                ...
                />
                <AppText style={styles.noCameraText}>
                  No active live cameras available.
                </AppText>
              </View>
            )}
          </View>
